...
from functionsaver.function_saver import replay_function_async
...
async def test_replay_with_output(reset_environment, fonctionsaver_in_tempfolder):
    ...
    for dir_path in Path(temp_folder).iterdir():
        if dir_path.is_dir():
            await replay_function_async(function_to_replay_without_internal, dir_path)
...
@pytest.mark.asyncio
async def test_replay_without_intenral_nor_output(
    reset_environment, fonctionsaver_in_tempfolder
):
    temp_folder = fonctionsaver_in_tempfolder
    await function_to_replay_without_internal_not_output(1, 2)

    for dir_path in Path(temp_folder).iterdir():
        if dir_path.is_dir():
            await replay_function_async(
                function_to_replay_without_internal_not_output, dir_path
            )
            assert_replay_output(dir_path, False, 1, 2)
            assert_replay_internal(dir_path, False, 1, 2)
            break
    else:
        assert False, "Cannot find a folder for replay"
...
@pytest.mark.asyncio
async def test_replay_without_output(reset_environment, fonctionsaver_in_tempfolder):
    temp_folder = fonctionsaver_in_tempfolder
    await function_to_replay_without_output(1, 2)

    for dir_path in Path(temp_folder).iterdir():
        if dir_path.is_dir():
            await replay_function_async(function_to_replay_without_output, dir_path)
            assert_replay_output(dir_path, False, 1, 2)
            assert_replay_internal(dir_path, True, 1, 2)
            await replay_function_async(
                function_to_replay_without_output_10times, dir_path
            )
            assert_replay_output(dir_path, False, 1, 2)
...
async def test_replay(reset_environment, fonctionsaver_in_tempfolder):
    ...
    for dir_path in Path(temp_folder).iterdir():
        if dir_path.is_dir():
            await replay_function_async(function_to_replay, dir_path)
            assert_replay_output(dir_path, True, 1, 2)
            assert_replay_internal(dir_path, True, 1, 2)
            await replay_function_async(function_to_replay_10times, dir_path)
