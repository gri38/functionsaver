...
def test_replay_with_output(reset_environment, fonctionsaver_in_tempfolder):
    ...
    for dir_path in Path(temp_folder).iterdir():
        if dir_path.is_dir():
            replay_function(function_to_replay_without_internal, dir_path)
...
def test_replay_without_intenral_nor_output(
    reset_environment, fonctionsaver_in_tempfolder
):
    temp_folder = fonctionsaver_in_tempfolder
    function_to_replay_without_internal_not_output(1, 2)

    for dir_path in Path(temp_folder).iterdir():
        if dir_path.is_dir():
            replay_function(function_to_replay_without_internal_not_output, dir_path)
...
def test_replay_without_output(reset_environment, fonctionsaver_in_tempfolder):
    temp_folder = fonctionsaver_in_tempfolder
    function_to_replay_without_output(1, 2)

    for dir_path in Path(temp_folder).iterdir():
        if dir_path.is_dir():
            replay_function(function_to_replay_without_output, dir_path)
            assert_replay_output(dir_path, False, 1, 2)
            assert_replay_internal(dir_path, True, 1, 2)
            replay_function(function_to_replay_without_output_10times, dir_path)
            assert_replay_output(dir_path, False, 1, 2)
...
def test_replay(reset_environment, fonctionsaver_in_tempfolder):
    ...
    for dir_path in Path(temp_folder).iterdir():
        if dir_path.is_dir():
            replay_function(function_to_replay, dir_path)
            assert_replay_output(dir_path, True, 1, 2)
            assert_replay_internal(dir_path, True, 1, 2)
            replay_function(function_to_replay_10times, dir_path)
